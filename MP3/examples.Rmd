...
```{r}
bert_cities <- tibble(
  name = c("Boston","Amherst", "Northampton"), 
  lat = c(42.3601, 42.370, 42.316), 
  lon = c(-71.0589,-72.516, -72.640))

USA_sf1 <- USA_sf %>% 
  filter(ID=="massachusetts")
bert_cities_sf <- bert_cities %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326)

ggplot() +
  geom_sf(data = USA_sf1) +
  geom_sf(data = bert_cities_sf, aes(col = name), size = 2) 
